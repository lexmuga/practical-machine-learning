---
title: "Quantifying the Weight Lifting Exercises Using R"
...
1. **Selection phase** where we select the best model of **Random Forest** on 3 variants using **10\%** of the data set. The 3 variants are:
    + **rfModel1 : Random Forest** with no preprocessing of the data **(5)**.
    + **rfModel2 : Random Forest** with the data preprocessed by **center** and **scale** **(6)**
    + **rfModel3 : Random Forest** with the data preprocessed by **pca** **(7)**
...
Finally, there are **`r ncol(pmlTraining)-1`** variables as **predictors** and one **outcome**, the variable **classe**.
...
1. **rfModel1 : Random Forest** with no preprocessing of the data,
2. **rfModel2 : Random Forest** with the data preprocessed by **center** and **scale**, and
3. **rfModel3 : Random forest** with the data preprocessed by **pca**.
...
```{r validatingRFNoPreProc, echo=FALSE, message=FALSE, eval=T}
set.seed(100200300)
...
rfModel1 <- train(classe ~ .,  
                ...
                prox=TRUE,
                allowParallel=TRUE)
save(rfModel1, file="rfModel1_03.RData")
```

```{r, echo=F, message=FALSE}
load("rfModel1_03.RData")
```

```{r preProcessWithCenterScale, echo=FALSE, message=FALSE, eval=T}
set.seed(100200300)
...
```{r validatingRFWithCS, echo=FALSE, message=FALSE, eval=T}
set.seed(100200300)
...
rfModel2 <- train(selection$classe ~ .,  
                ...
                prox=TRUE,
                allowParallel=TRUE)
save(rfModel2, file="rfModel2_03.RData")
```

```{r, echo=F, message=FALSE}
load("rfModel2_03.RData")
```

```{r preProcessWithPCA, echo=FALSE, message=FALSE, eval=T}
set.seed(100200300)
...
```{r validatingRFWithPCA, echo=FALSE, message=FALSE, eval=T}
set.seed(100200300)
...
rfModel3 <- train(selection$classe ~ .,  
                ...
                prox=TRUE,
                allowParallel=TRUE)
save(rfModel3, file="rfModel3_03.RData")
```

```{r, echo=FALSE, message=FALSE}
load("rfModel3_03.RData")
```
...
resampRFs <- resamples(list(rfModel1,
                            rfModel2,
                            rfModel3))
summary(resampRFs)$statistics
```

There is a tie for the best model between the **First Model (rfModel1)** and the **Second Model (rfModel2)** as shown in the **Accuracy** and **Kappa** metrics in **resampling**.
See the figure generated by **bwplot** below.

```{r, echo=FALSE, message=FALSE, include=FALSE}
png(file="bwplot.png", width=560, height = 300)
bwplot(resampRFs)
dev.off()
```
...
It is confirmed by the $\mathbf{\text{p-value} > 0.05}$ calculated by the **diff** function in both metrics. See the table below. So we cannot reject the null hypothesis that the respective **Accuracy** and **Kappa** of the **First Model** and **Second Model** are equal.
...
We choose the **rfModel1** as our best model since it is simpler to build than the **rfModel2**.

## G. Training Phase

We shall now train our best model with **70\%** of the **pmlTraining** data set.

```{r trainingBestModel, echo=FALSE, message=FALSE, eval=T}
set.seed(100200300)
...
save(rfBestModel, file="rfBestModel03.RData")
```

```{r, echo=F, message=FALSE}
load("rfBestModel03.RData")
```
...
```{r, echo=FALSE, message=FALSE}
IOSError <- sum(cm)/nrow(training)
```

* **The number of wrong predictions is `r sum(cm)` out of the `r prettyNum(nrow(training),big.mark=",")` total possible outcomes.**

* Therefore, the **in-sample error**  of our **Best Random Forest Model** after training with $\mathbf{70\%}$ of the **pmlTraining** data set is **`r round(100*IOSError,2)`\%.**

The accuracy rating of our **Best Random Forest Model** after the training is **`r round(rfBestModel$results[1,2]*100,2)`\%**.

## H. Testing Phase

Using the **testing** data set which is $\mathbf{20\%}$ of the **pmlTraining** data set for our **Testing Phase**, we predicted the following results as shown in the figure below.
...
diag(cMat) <- 0
testingError <- round(sum(cMat)/length(results2),3)
accuracy <- as.numeric(cMatrix2$overall[1])
kappa <- as.numeric(cMatrix2$overall[2])
```

The confusion matrix of the **Random Forest BestModel** on the **classe** variable of the **testing** data set  shows an **Accuracy rating of `r round(accuracy*100,2)`\%** and the **Kappa rating of `r round(kappa*100,2)`\%**.

* **The number of mismatches is `r sum(cMat)` out of `r prettyNum(length(results2),big.mark=",")` Predictions.** 

# I. On the pmlTesting Data Set

The **pmlTesting** data set has only 20 observations. The predictions of the **Best Random Forest Model** on the **pmlTesting** data set is given below.

```{r, echo=FALSE, message=FALSE}
resultPMLTesting <- predict(rfBestModel, pmlTesting)
res <- data.frame(problemId = pmlTesting$problem_id,
                  classe=resultPMLTesting)
res
```

We have the frequency distribution of our predicted values on the **pmlTesting** data set.

```{r, echo=FALSE, message=FALSE}
table(resultPMLTesting)
```

The prediction has a perfect accuracy on the **pmlTesting** data set since we got a score of **20/20** in the assignment part of this project. See the assignment result below.

![](assignmentScore.png)


# J. Out-of-Sample Error with 2-Fold Cross Validation

Our **computed in-sample error** is about **`r round(sum(cMat)/length(results2)*100,1)`\%**. We expect that **out-of-sample error** is larger than this value.

**Out-of-sample error** or the **generalization error** as defined in the lecture of Professor Leek is the "error you get on a new data set". **(15)**

We shall be using **2-Fold Cross Validation** to determine the **out-of-sample error** on the **plmTraining** data set in the absence of a large new data set.  

This is the simplest of the **k-Fold Cross Validation** where we randomly partitioned the **pmlTraining** data set into two equal sizes, **Fold01** and **Fold02**. In the first cross-validation, we train the model on **Fold01** and test on **Fold02**. In the second cross-validation, we  train the model on **Fold02** and test on **Fold01**. See the Table below.

```{r creatingTwoFolds, echo=FALSE, message=FALSE}
library(caret)
set.seed(100200300)
kFolds <- createFolds(pmlTraining$classe, 2)
Fold01 <- pmlTraining[kFolds[[1]],]
Fold02 <- pmlTraining[kFolds[[2]],]
```

```{r creatingTableFolds, echo=FALSE, message=FALSE}
tableFolds <- data.frame(rbind(table(Fold01$classe), 
                               table(Fold02$classe)
                               )
                         )
```

```{r, preparingTestingTrainingData, echo=FALSE, message=FALSE}
testing01 <- Fold01
training01 <- Fold02
testing02 <- Fold02
training02 <- Fold01
...
```{r creatingCVTable01, echo=FALSE, message=FALSE}
crossValidation <- c("cv01","cv02")
Fold01 <- c("test","train")
Fold02 <- c("train","test")
cvTable01 <- data.frame(crossValidation, Fold01, Fold02)
cvTable01
```

Our training and test sets are both relatively large (50\% each) and we used each data point in both the training and testing stages.

```{r creatingCVTable2, echo=FALSE, message=FALSE}
cvTestingSize <- c(nrow(testing01), 
                   nrow(testing02)
                   )
cvTrainingSize <- c(nrow(training01),
                    nrow(training02)
                    )
cvTable02 <- data.frame(crossValidation, cvTestingSize, cvTrainingSize)
cvTable02
```

We shall call the final model trained in the first cross-validation as **cv01Model** and the final model trained in the second cross-validation as **cv02Model**.

```{r, cv01Training, echo-FALSE, message=FALSE, include=FALSE, eval=T}
cv01Model <- train(classe ~.,
                   data=training01,
                   method="rf",
                   trControl = fitControl,
                   prox=TRUE,
                   allowParallel=TRUE)
save(cv01Model, file="cv01Model.RData")
```

```{r, cv02Training, echo-FALSE, message=FALSE, include=FALSE, eval=T}
cv02Model <- train(classe ~.,
                   data=training02,
                   method="rf",
                   trControl = fitControl,
                   prox=TRUE,
                   allowParallel=TRUE)
save(cv02Model, file="cv02Model.RData")
```

```{r loadingCVModels, echo=FALSE, message=FALSE}
load("cv01Model.RData")
load("cv02Model.RData")
```

```{r cvPrediction, echo-FALSE, message=FALSE, include=FALSE}
cv01Prediction <- predict(cv01Model, testing01)
cv02Prediction <- predict(cv02Model, testing02)
```

The confusion matrix of the predicted values of **cv01Model** and the **classe** variable of **testing01** data set in the first cross-validation is given below:

```{r cvConfusionMatrix, echo-FALSE, message=FALSE, include=FALSE}
confusionMatrix01 <- confusionMatrix(cv01Prediction, testing01$classe)
confusionMatrix02 <- confusionMatrix(cv02Prediction, testing02$classe)
cMat01 <- matrix(confusionMatrix01$table, nrow=5, ncol=5)
cMat02 <- matrix(confusionMatrix02$table, nrow=5, ncol=5)
diag(cMat01) <- 0
diag(cMat02) <- 0
cv01ModelMismatch <- sum(cMat01)
cv02ModelMismatch <- sum(cMat02)
OOSError <- (cv01ModelMismatch+cv02ModelMismatch)/(nrow(testing01)+nrow(testing02))
```

```{r confusionMatrix01, echo=FALSE, message=FALSE}
confusionMatrix01$table
```

This implies that the **cv01Model** was wrong in predicting **`r cv01ModelMismatch` outcomes**.

The confusion matrix of the predicted values of **cv02Model** and the **classe** variable of **testing02** data set of the second cross-validation is given below:

```{r confusionMatrix02, echo=FALSE, message=FALSE}
confusionMatrix02$table
```

This implies that the **cv02Model** was wrong in predicting **`r cv02ModelMismatch` outcomes**.


Therefore, the **Out-of-Sample Error** is equal to **(`r cv01ModelMismatch` + `r cv02ModelMismatch`)/(`r prettyNum(nrow(pmlTraining), big.mark=",")`) = `r round(100*OOSError,3)`\%**.
