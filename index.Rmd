...
Finally, there are **`r ncol(pmlTraining)`** variables which we shall consider initially as **predictors** and one of them is the outcome, **classe**.
...
We choose the **First Model (no preProc)** as our best model since it is simpler to build than the **Second Model (with 'center-scale')**.
...
Predicting the **pmlTesting** data set with the **Best Random Forest Model**, we obtained.
...
The frequency distribution of our predicted values on the **pmlTesting** data set is given by:

```{r, echo=FALSE, message=FALSE}
table(resultPMLTesting)
```

The  accuracy of this prediction is confirmed when we obtained a score of **20/20** in the assignment part of this project.
